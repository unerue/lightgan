...
class StyleGAN2Discriminator(nn.Module):
    ...
    def forward(self, input, t, get_minibatch_features=False):
...
class StyleGAN2Decoder(nn.Module):
    def __init__(
        ...
        # conv_init2 = nn.ModuleList()
        for i in range(n_blocks // 2):
            n_channel = channels[cur_res]
            # conv_init1.append(ResBlock(n_channel, n_channel, downsample=False))
            conv_init.append(
                StyledConv(
                    n_channel,
                    n_channel,
        ...
        self.conv_early = nn.Sequential(*conv_init)
        # self.conv_early2 = nn.Sequential(*conv_init2)
        self.convs = nn.Sequential(*convs)

    def forward(self, input, style, temb):
        out = input
        # for layer in self.conv_early:
        #     out = layer(out)
        for layer in self.conv_early:
            out = layer(out, style, temb)
        # out = self.conv_early(input,style,temb)
        out = self.convs(out)
        return out
